...
sip_parse(const char *str, int len, sip_attrs_t *attr_out)
{
	...
	bzero(attr_out->via, sizeof(int)*7);
	bzero(attr_out->credential, sizeof(int)*16);
	// 20090302
	bzero(attr_out->sdp, sizeof(int)*116);
	attr_out->expires = 0;
...
//			case 'I':	/* for INVITE case */
			...
			case 'V':
				if (strncmpi(via_str,attr,alen) == 0 ^ strncmpi(via_sstr,attr,alen) == 0) {
					...
					probe = val;
					probeend = val + vlen;
					attr_out->via->sent_protocol = probe;
					while (probe < probeend && *probe != ' ') ++probe;
					attr_out->via->sent_protolen = probe - attr_out->via->sent_protocol;
					++probe;
					/* Via sent-by host */
					attr_out->via->host = probe;
					while (probe < probeend && *probe != ':' && *probe != ';') ++probe;
					attr_out->via->hostlen = probe - attr_out->via->host;
					if(*probe == ':') {
						/* Via sent-by port */
						++probe;
						attr_out->via->port = atoi(probe);
						while (probe < probeend && *probe != ';') ++probe;
					}
					++probe;
					/* via-params */
						...
						switch(*attr) {
							case 'b':	/* Via - branch */
								if(strncmpi(via_branch_str,attr,alen) == 0) {
									attr_out->via->branch = val;
									attr_out->via->branchlen = vlen;
								}
								break;
							case 'm':	/* Via - branch */
								if(strncmpi(via_maddr_str,attr,alen) == 0) {
									attr_out->via->maddr = val;
									attr_out->via->maddrlen = vlen;
								}
								break;
							case 'r':	/* Via - branch */
								if(strncmpi(via_received_str,attr,alen) == 0) {
									attr_out->via->received = val;
									attr_out->via->receivedlen = vlen;
								}
								break;
							case 't':	/* Via - ttl */
								if(strncmpi(via_ttl_str,attr,alen) == 0) {
									attr_out->via->ttl = atoi(val);
								}
								break;
						}
						if(probe >= probeend) break;
...
/*			probe = attr;
		...
		switch (*attr) {	
			...
			case 'a':	/* attribute-field */
				attr_out->a[a_count] = val;
				attr_out->alen[a_count] = vlen;
				a_count++;
				break;
