...
typedef struct phone_pool
{
	char phone_number[64];
	char src_ip[32];
	char dst_ip[32];
	int reg[8];	/* 0 for request, 7 for other */
	int inv[8];	/* 0 for request, 7 for other */
	int other;
} phone_number_pool;

typedef struct old_summary
{
	char src_ip[32];
	char dst_ip[32];
	long reg_req;
...
typedef struct summary
{
	char src_ip[32];
	char dst_ip[32];
	unsigned int reg[7][10][10];
	unsigned int inv[7][10][10];
	unsigned int q850_cause[256];
} statistics;
