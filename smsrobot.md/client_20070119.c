...
const char *calliduri, *contacturi, *fromid, *toid, *fromuri, *touri; 
// const char *maxfwd, *expires, *conlength;
int mlen, flen, tlen, clen, vlen, ilen, slen, ulen, alen;
int maxfwd, expires, contextlen;
int callidurilen, contacturilen, fromlen, tolen, fromurilen, tourilen;
char ncbuf[9];
} sip_attrs_t;
...
void * calcreponse(char *resp, digest_attrs_t *sipauth);
...
void * recv_message(void *arg)
{
    ...
    char defaultauthuri[] = "sip:samsung070.com";
    // char defaultcnonce[] = "176872";
    char defaultcnonce[] = "";
    char *rcvdresp;
    ...
    rcvdsipauth->cnonce = 100;
    rcvdresp = (char*)malloc(sizeof(char) * 128);
    // rcvdsipauth->resp = respstr;
    if(-5 == sip_parse(message, strlen(message), rcvdsip)) printf("abnormal parsing error.\n");
    /* set auth-default */
    rcvdsipauth->uri = defaultauthuri;
    rcvdsipauth->urilen = strlen(defaultauthuri);
    rcvdsipauth->cnonce = defaultcnonce;
    rcvdsipauth->clen = strlen(defaultcnonce);
...
//    printf("NONCE:%s\n", rcvdsipauth->nonce);
    ...
    fflush(stdout);
    */
    // printf("Authorization: Digest username=\"%s\",realm=\"%s\",nonce=\"%s\",uri=\"%s\"\r\n", rcvdsip->fromid, rcvdsipauth->realm, rcvdsipauth->nonce, rcvdsipauth->uri); //, rcvdsipauth->resp, rcvdsipauth->cnonce);
    // printf("Authorization: Digest username=\"%s\",realm=\"%s\",nonce=\"%s\",uri=\"%s\",response=\"%s\",algorithm=MD5,cnonce=\"%d\",nc=00000001,qop=auth\r\n", rcvdsip->fromid, rcvdsipauth->realm, rcvdsipauth->nonce, rcvdsipauth->uri, rcvdsipauth->resp, rcvdsipauth->cnonce);
    // fflush(stdout);
    }
...
void* calcreponse(char *resp, digest_attrs_t *sipauth)
{
char * pszNonce; // = sipauth->nonce;
char * pszCNonce; // = "253194";
char * pszUser = "07070156894";
char * pszRealm = "203.254.210.100";
char * pszPass = "112880";
char * pszAlg = "MD5";
char szNonceCount[9] = "00000001";
char * pszMethod = "REGISTER";
char * pszQop = "auth";
char * pszURI = "sip:samsung070.com";
HASHHEX HA1;
HASHHEX HA2 = "";
const HASHHEX Response;

// pszNonce = sipauth->nonce;
pszNonce = (char*)malloc(sizeof(char) * (sipauth->nlen + 1));
sprintf(pszNonce,"%.*s", sipauth->nlen, sipauth->nonce);
// printf("[[%s]]\n", pszNonce);
pszCNonce = sipauth->cnonce;
// printf("[[%s]]\n", sipauth->cnonce);
...
DigestCalcResponse(HA1, pszNonce, szNonceCount, pszCNonce, pszQop, pszMethod, pszURI, HA2, resp);
// printf("DIGEST END\n");
//printf("RESPONSE:%s\n", Response);
//printf("RESPONSE:%s\n", resp);
//fflush(stdout);
...
// resp = (char*)Response;
// sprintf(Response, "Authorization: Digest username=\"%s\",realm=\"%s\",nonce=\"%s\",uri=\"%s\",response=\"%s\",algorithm=MD5,cnonce=\"%d\",nc=00000001,qop=auth\r\n", pszUser, pszRealm, pszNonce, pszURI, , auth->cnonce);
}
...
build_register(char *msg, const char *srcip, int srcport, const sip_attrs_t *sip, const digest_attrs_t *auth)
{
    ...
    */
    ...
    /*
    memcpy(pszFromURI, sip->fromuri, sip->fromurilen);
    memcpy(pszFromID, sip->fromid, sip->fromlen);
    memcpy(pszToURI, sip->touri, sip->tourilen);
    memcpy(pszToID, sip->toid, sip->tolen);
    pszFromURI[sip->fromurilen] = 0;
    pszFromID[sip->fromlen] = 0;
    pszToURI[sip->tourilen] = 0;
    pszToID[sip->tolen] = 0;
    */
...
//    printf("##############\n%s#################\n", msg);
    sprintf(msg, "REGISTER sip:%.*s SIP/2.0\r\n", sip->fromurilen, sip->fromuri);
    sprintf(start + strlen(msg), "From: <sip:%.*s@%.*s>;tag=23-00000000\r\n", sip->fromlen, sip->fromid, sip->fromurilen, sip->fromuri);
    sprintf(start + strlen(start), "To: <sip:%.*s@%.*s>\r\n", sip->tolen, sip->toid, sip->tourilen, sip->touri);
    sprintf(start + strlen(start), "Contact: <sip:%.*s@%.*s:%d>\r\n", sip->fromlen, sip->fromid, sip->callidurilen, sip->calliduri, srcport);
    sprintf(start + strlen(start), "Via: SIP/2.0/UDP %.*s:%d;branch=z9hG4bK-1\r\n", sip->callidurilen, sip->calliduri, srcport);
    sprintf(start + strlen(start), "Call-ID: 23-00000000@%.*s\r\n", sip->callidurilen, sip->calliduri);
    sprintf(start + strlen(start), "CSeq: %d REGISTER\r\n", cseq);
    sprintf(start + strlen(start), "User-Agent: XKSTYLE-SMS-ROBOT\r\n");
    sprintf(start + strlen(start), "Max-Forwards: 70\r\n");
    sprintf(start + strlen(start), "Expires: 3600\r\n");

    /*
    printf("%.*s\n", sip->fromlen, sip->fromid);
    printf("%.*s\n", sip->fromurilen, sip->fromuri);
    printf("%.*s\n", auth->rlen, auth->realm);
    printf("%.*s\n", auth->nlen, auth->nonce);
    printf("%.*s\n", auth->urilen, auth->uri);
    printf("%.*s\n", auth->resplen, auth->resp);
    printf("%.*s\n", auth->clen, auth->cnonce);
    fflush(stdout);
    */

//    sprintf(start + strlen(start), "Authorization: Digest username=\"%.*s\",realm=\"%.*s\",nonce=\"%.*s\",uri=\"%.*s\",response=\"%s\",algorithm=\"MD5\",cnonce=\"%d\",nc=\"00000001\",qop=\"auth\"\r\n", sip->fromlen, sip->fromid, auth->rlen, auth->realm, auth->nlen, auth->nonce, auth->urilen, auth->uri, auth->resplen, auth->resp, auth->clen, auth->cnonce);
    sprintf(start + strlen(start), "Authorization: Digest username=\"%.*s\",realm=\"%.*s\",nonce=\"%.*s\",", sip->fromlen, sip->fromid, auth->rlen, auth->realm, auth->nlen, auth->nonce);
    /*
    sprintf(start + strlen(start), "uri=\"%.*s\",response=\"%.*s\",algorithm=MD5,cnonce=\"%.*s\",nc=00000001,qop=auth\r\n", auth->urilen, auth->uri, auth->resplen, auth->resp, auth->clen, auth->cnonce);
     */
    sprintf(start + strlen(start), "uri=\"%.*s\",response=\"%.*s\",algorithm=MD5,nc=00000001,qop=\"auth\"\r\n", auth->urilen, auth->uri, auth->resplen, auth->resp);
    sprintf(start + strlen(start), "Content-Length: 0\r\n\r\n");
//    sprintf(msg, "REGISTER sip:%s SIP/2.0\r\n", sip->fromuri);
    /*
    sprintf(msg, "REGISTER sip:%s SIP/2.0\r\n", sip->fromuri);
		...
		switch (*attr) {
			...
			//	attr_out->ilen = vlen;
	                    ...
	                    while (probe < probeend && *probe != '@') ++probe;
                            attr_out->ilen = probe - attr_out->callid;
                            ++probe;
