...
typedef struct {
const char *method, *uri, *caller, *callee, *callid, *useragt, *auth, *context, *result;
int methodlen, urilen, useridlen, callerlen, calleelen, callidlen, useragtlen, authlen, contextlen, resultlen;
...
sip_parse(const char *str, int len, sip_attrs_t *attr_out)
{
	...
	static const char statusstr[] = "SIP/2.0";
	...
	for (;;) {
		...
		switch (*attr) {
		    ...
		    case 's':
		    case 'S':
			if (memcmp(statusstr,attr,alen) == 0) {
				attr_out->result = val;
				attr_out->resultlen = vlen;	
			}
			break;
		    case 't':
		    case 'T':
